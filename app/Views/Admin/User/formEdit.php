...
<div class="p-5">
    ...
    <form class="user" action="<?php echo base_url(); ?>/Admin/User/updateData/<?= $user['user_id']; ?>" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control <?= ($validation->hasError('nik') ? 'is-invalid' : ''); ?>" id="nik" name="nik" value="<?= (old('name') ? old('nik') : $user['nik']); ?>">
...
<script>
    $('#jenis_kelamin').val('<?= (old('jenis_kelamin') ? old('jenis_kelamin') : $user['jenis_kelamin']); ?>').trigger('change');
    $('#jabatan_id').val('<?= (old('jabatan_id') ? old('jabatan_id') : $user['jabatan_id']); ?>').trigger('change');
</script>
